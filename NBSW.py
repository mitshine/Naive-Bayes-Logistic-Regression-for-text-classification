...
def loadTrainingData(path, stopwords):
    ...
    for file in files:
        ...
        wordsFromFile = [selectedWord for selectedWord in wordsFromFile if selectedWord.lower() not in stopwords]
...
def classifyLabel(condProb, prior, label, path, actualLabel, stopwords):
    files = glob.glob(path)
    correctPred = 0
    incorrectPred = 0
    x = 0
    for file in files:
        ...
        words = [selectedWord for selectedWord in words if selectedWord.lower() not in stopwords]
...
def main():
    ...
    trainHam, trainHamCount = loadTrainingData(hamTrainPath, stopwords)
    trainSpam, trainSpamCount = loadTrainingData(spamTrainPath, stopwords)
    ...
    maxHamScore, hamCorrectPred, hamIncorrectPred = classifyLabel(condProb, prior, label, hamTestPath, label[0], stopwords)
    hamAccuracy = round((hamCorrectPred / (hamCorrectPred + hamIncorrectPred)) * 100, 2)

    #Calculate Accuracy of SPAM data.
    maxSpamScore, spamCorrectPred, spamIncorrectPred = classifyLabel(condProb, prior, label, spamTestPath, label[1], stopwords)
